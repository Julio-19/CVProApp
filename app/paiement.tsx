...
export default function PaiementScreen() {
  ...
  const getNomTemplate = () =>
    templateId?.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase()) ?? "Template";
  ...
  const lancerPaiement = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("create-payment", {
        body: {
          templateId, montant: prix, userId: user.id,
      ...
      if (error || !data?.paymentUrl) {
        throw new Error(error?.message ?? `Réponse invalide: ${JSON.stringify(data)}`);
      }

      setPaymentUrl(data.paymentUrl);
      setTransactionId(String(data.transactionId));

      // Sur web, ouvrir dans nouvel onglet
      if (Platform.OS === 'web') {
      ...
      if (data?.confirme === true) {
        ...
        Alert.alert(
          ...
          [
            ...
            { text: "Annuler", style: "cancel", onPress: () => setEtape("choix") },
          ]
        );
      }
    } catch (err: any) {
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>
          {etape === "succes" ? "✅ Paiement confirmé"
            : etape === "processing" ? "⏳ Vérification..."
            : "Paiement sécurisé"}
        </Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <Animated.View style={{ opacity: fadeAnim, transform: [{ scale: scaleAnim }] }}>
            ...
            <>
              <View style={styles.orderCard}>
                <View style={styles.orderHeader}>
                  ...
                  <View style={styles.orderInfo}>
                    ...
                    <Text style={styles.orderSub}>Template CV professionnel</Text>
                  </View>
                  <View style={styles.orderPrixContainer}>
                    <Text style={styles.orderPrix}>{prix.toLocaleString()}</Text>
                ...
                <View style={styles.orderRow}>
                  <Text style={styles.orderLabel}>Paiement sécurisé FedaPay</Text>
                  <Text style={styles.orderCheck}>✓</Text>
                </View>
              </View>
              ...
              <View style={styles.securityInfo}>
                <Text style={styles.securityText}>
                  🔒 Paiement 100% sécurisé via FedaPay · SSL · Données chiffrées
                </Text>
              </View>
              ...
              >
                {loading
                  ...
                  : <Text style={styles.btnPayerText}>Payer {prix.toLocaleString()} XOF →</Text>
                }
              </TouchableOpacity>
          ...
          {etape === "processing" && (
            <View style={styles.processingContainer}>
              ...
              {paymentUrl && (
                ...
                >
                  <Text style={styles.btnReouvrirText}>↗ Rouvrir la page de paiement</Text>
                </TouchableOpacity>
              )}
              <TouchableOpacity
              ...
              >
                {verifying
                  ? <ActivityIndicator color="#fff" size="small" />
                  : <Text style={styles.btnVerifierText}>✓ J'ai payé — Vérifier le paiement</Text>
                }
              </TouchableOpacity>
          ...
          {etape === "succes" && (
            <View style={styles.successContainer}>
              ...
              <Text style={styles.successSub}>
                Le template <Text style={styles.bold}>{getNomTemplate()}</Text> est maintenant actif.
              </Text>
              <View style={styles.successDetails}>
                ...
                <View style={styles.successRow}>
                  ...
                  <Text style={styles.successValue}>{prix.toLocaleString()} XOF</Text>
                </View>
                <View style={styles.successRow}>
                  <Text style={styles.successLabel}>Statut</Text>
                  <Text style={[styles.successValue, { color: "#16a34a" }]}>✓ Confirmé</Text>
                </View>
              </View>
              <TouchableOpacity style={styles.btnGenerer} onPress={() => router.push("/saved")}>
                <Text style={styles.btnGenererText}>Générer mon CV →</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => router.push("/templates")}>
                <Text style={styles.annulerLink}>Acheter un autre template</Text>
              </TouchableOpacity>
            </View>
          )}
          ...
          {etape === "echec" && (
            <View style={styles.successContainer}>
              <Text style={styles.successEmoji}>❌</Text>
              <Text style={[styles.successTitle, { color: "#dc2626" }]}>Paiement échoué</Text>
              <Text style={styles.successSub}>
                Le paiement n'a pas pu être confirmé. Vérifiez votre solde et réessayez.
              </Text>
              <TouchableOpacity style={styles.btnGenerer} onPress={() => setEtape("choix")}>
...
const styles = StyleSheet.create({
  ...
  orderCard:            { backgroundColor: "#fff", borderRadius: 20, padding: 20, gap: 12, elevation: 3, shadowColor: "#000", shadowOpacity: 0.08, shadowOffset: { width: 0, height: 2 }, shadowRadius: 8 },
  ...
  securityInfo:         { backgroundColor: "#f0fdf4", borderRadius: 12, padding: 12, borderWidth: 1, borderColor: "#bbf7d0" },
  securityText:         { fontSize: 11, color: "#15803d", textAlign: "center", lineHeight: 16 },
  btnPayer:             { backgroundColor: "#534AB7", borderRadius: 16, paddingVertical: 18, alignItems: "center", elevation: 4, shadowColor: "#534AB7", shadowOpacity: 0.35, shadowOffset: { width: 0, height: 4 }, shadowRadius: 8 },
  btnPayerText:         { color: "#fff", fontSize: 17, fontWeight: "800", letterSpacing: 0.5 },
  btnDisabled:          { opacity: 0.55 },
  redirectInfo:         { textAlign: "center", fontSize: 11, color: "#aaa", marginTop: -4 },
  processingContainer:  { alignItems: "center", padding: 16, gap: 20, marginTop: 24 },
  processingIconBg:     { width: 100, height: 100, borderRadius: 50, backgroundColor: "#fef3c7", alignItems: "center", justifyContent: "center" },
  processingEmoji:      { fontSize: 48 },
  processingTitle:      { fontSize: 22, fontWeight: "800", color: "#111" },
  processingSub:        { fontSize: 14, color: "#555", textAlign: "center", lineHeight: 22 },
  btnReouvrir:          { backgroundColor: "#f0f0f0", borderRadius: 14, paddingVertical: 14, paddingHorizontal: 24, width: "100%", alignItems: "center" },
  btnReouvrirText:      { fontSize: 14, color: "#534AB7", fontWeight: "600" },
  btnVerifier:          { backgroundColor: "#534AB7", borderRadius: 16, paddingVertical: 18, width: "100%", alignItems: "center", elevation: 3 },
  btnVerifierText:      { color: "#fff", fontSize: 15, fontWeight: "700" },
  annulerLink:          { textAlign: "center", color: "#aaa", fontSize: 13, marginTop: 4 },
  successContainer:     { alignItems: "center", padding: 16, gap: 20, marginTop: 24 },
  successIconBg:        { width: 100, height: 100, borderRadius: 50, backgroundColor: "#f0fdf4", alignItems: "center", justifyContent: "center" },
  ...
  successDetails:       { backgroundColor: "#fff", borderRadius: 16, padding: 20, gap: 12, width: "100%", elevation: 2 },
  ...
  btnGenerer:           { backgroundColor: "#16a34a", borderRadius: 16, paddingVertical: 18, width: "100%", alignItems: "center", elevation: 3 },
